"""Tests for visual system: MiroFish, Remotion, Radiant, AssetManifest."""

import pytest
from unittest.mock import AsyncMock, patch

# ─── MiroFish seeds (pure functions) ─────────────────────────────────────────

from packages.integrations.mirofish.seeds import (
    create_geopolitical_seed,
    create_tech_trend_seed,
    create_audience_forecast,
    create_combined_seed,
)


def test_geopolitical_seed_contains_events():
    seed = create_geopolitical_seed(["Pakistan AI policy announced"])
    assert "Pakistan AI policy announced" in seed
    assert "South Asia" in seed


def test_tech_seed_contains_developments():
    seed = create_tech_trend_seed(["GPT-5 released"], focus="AI")
    assert "GPT-5 released" in seed
    assert "AI" in seed


def test_audience_forecast_contains_audience():
    fc = create_audience_forecast(audience="Pakistani viewers")
    assert "Pakistani viewers" in fc


def test_combined_seed_returns_tuple():
    seed, forecast = create_combined_seed(
        geopolitical=["Event A"],
        tech=["Tech B"],
    )
    assert isinstance(seed, str)
    assert isinstance(forecast, str)
    assert "Event A" in seed
    assert "Tech B" in seed


# ─── MiroFish client (mocked HTTP) ───────────────────────────────────────────

@pytest.mark.asyncio
async def test_mirofish_returns_none_on_connect_error():
    ...
    with patch.object(client._http, "post",
        ...
        result = await client.submit_seed("seed", "forecast")
    assert result is None
    await client.close()


@pytest.mark.asyncio
async def test_mirofish_get_status_returns_unknown_on_error():
    from packages.integrations.mirofish.client import MiroFishClient
    import httpx
    client = MiroFishClient()
    with patch.object(client._http, "get",
                      side_effect=httpx.ConnectError("refused")):
        result = await client.get_status("sim-123")
    assert result == {"status": "unknown"}
    await client.close()


@pytest.mark.asyncio
async def test_mirofish_interact_returns_empty_on_error():
    from packages.integrations.mirofish.client import MiroFishClient
    import httpx
    client = MiroFishClient()
    with patch.object(client._http, "post",
                      side_effect=httpx.ConnectError("refused")):
        result = await client.interact("sim-123", "What trends?")
    assert result == ""
    await client.close()
